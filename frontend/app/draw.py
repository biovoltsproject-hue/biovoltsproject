...
# ICONE DA PLACA SOLAR
def icon_weather(canvas, cx, cy):
    canvas.create_oval(cx-10, cy-14, cx+14, cy+14, fill="", outline=YELOW, width=3)
    for i in range(8):
        angle = i * (math.pi / 4)
        ...
        canvas.create_line(x1, y1, x2, y2, fill=YELOW, width=3)
...
def icon_plug(canvas, cx, cy):
    ...
    canvas.create_polygon(pts, fill="", outline=WHITE, width=3)
    canvas.create_line(cx-pw//2, cy-ph//2+6, cx+pw//2, cy-ph//2+6, fill=WHITE, width=2)

    # Pinos
    canvas.create_line(cx-6, cy-ph//2, cx-6, cy-ph//2-12, fill=WHITE, width=3)
    canvas.create_line(cx+6, cy-ph//2, cx+6, cy-ph//2-12, fill=WHITE, width=3)

    # Fio
    canvas.create_line(cx, cy+ph, cx, cy+ph+16, fill=WHITE, width=3)

def icon_clock(canvas, cx, cy):
    canvas.create_oval(cx-10, cy-10, cx+10, cy+10, fill="", outline=WHITE, width=2)
    canvas.create_line(cx, cy, cx, cy-8, fill=WHITE, width=2)
    canvas.create_line(cx, cy, cx+10, cy+6, fill=WHITE, width=2)
